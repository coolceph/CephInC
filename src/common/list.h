...
#define CCEPH_POISON_POINTER_DELTA 0

#define CCEPH_LIST_POISON1  ((char *) 0x00100100 + CCEPH_POISON_POINTER_DELTA)
#define CCEPH_LIST_POISON2  ((char *) 0x00200200 + CCEPH_POISON_POINTER_DELTA)
...
static inline void cceph_init_list_head(struct cceph_list_head *list)
{
    list->prev = list;
    list->next = list;
};
...
static inline  void __cceph_list_del(struct cceph_list_head *prev, struct cceph_list_head *next)
{
    prev->next = next;
    next->prev = prev;
};

static inline void cceph_list_del(struct cceph_list_head *entry)
{
    __cceph_list_del(entry->prev, entry->next);
...
{
        __cceph_list_del(list->prev, list->next);
